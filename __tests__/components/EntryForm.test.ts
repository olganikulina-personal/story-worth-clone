...
import { hasPendingAutosaveContent, shouldSkipAutosaveTransition } from "@/components/EntryForm";
...
describe("hasPendingAutosaveContent", () => {
  it("treats an empty string as pending autosave work", () => {
    expect(hasPendingAutosaveContent("")).toBe(true);
  });

  it("treats null as no pending autosave work", () => {
    expect(hasPendingAutosaveContent(null)).toBe(false);
  });
});
