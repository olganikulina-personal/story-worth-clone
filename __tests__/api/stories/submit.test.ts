...
function singleChain(result: { data: unknown; error: unknown }) {
  ...
  const methods = ['select', 'eq', 'gt', 'insert', 'update', 'limit']
...
function resolvingChain(result: unknown) {
  ...
  const chain: Record<string, unknown> = {}
  const methods = ['select', 'eq', 'gt', 'insert', 'update', 'limit', 'single']
  for (const m of methods) {
    chain[m] = vi.fn().mockReturnValue(chain)
  }
...
describe('POST /api/stories/submit', () => {
  ...
  // ---------------------------------------------------------------------------
  it('inserts story, marks token used, sends email, returns 200 for a fresh token', async () => {
  ...
  // ---------------------------------------------------------------------------
  it('updates story content and returns 200 when token is already used', async () => {
    ...
    const lockCheck = resolvingChain({ count: 0, error: null })
    // Call 3: update story → success
    const storyUpdate = resolvingChain({ error: null })

    fromMock
      .mockReturnValueOnce(tokenLookup)
      .mockReturnValueOnce(lockCheck)
      .mockReturnValueOnce(storyUpdate)
